...
const createOrder = (request, response, cartId, items, cart_total) => {
  const stato = "ricevuto"
  pool.query('INSERT INTO orders (status, cart_id, cart_total, items) VALUES ($1, $2, $3, $4)', [stato, cartId, cart_total, items], (error, results) => {
    if (error) {
      throw error
    }
  })
